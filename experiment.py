...
def pretrainModel(name, pretrain_iter, preval_iter, adj, device):
    ...
    model = Contrastive_FeatureExtractor_conv(P.TEMPERATURE, P.is_GCN).to(device)
    # min_val_loss = np.inf
    ...
    optimizer.zero_grad()
    loss = model.contrast(x[0].to(device), edge_masking(adj, 0.02, device), edge_masking(adj, 0.02, device)) # contrastive loss is generated by the model itself
    loss.backward()
    optimizer.step()
...
def trainModel(name, mode, 
    ...
    encoder = Contrastive_FeatureExtractor_conv(P.TEMPERATURE, P.is_GCN).to(device)
    encoder.eval()
    with torch.no_grad():
